...
const docMeta = computed(() => {
  ...
  if (!m) return { authority: '', date: '' }
  const authority = (m.issuing_authority || '').replace(/\n/g, ' ').trim()
  const date = m.issue_date || ''
  return { authority, date }
})
</script>
    ...
    <div class="p-3 border-b th-border flex items-center justify-between"
      ...
      <div class="flex items-center gap-2 min-w-0">
        ...
        <div class="min-w-0">
          ...
          <p v-if="docMeta.authority || docMeta.date" class="text-[11px] truncate" style="color: var(--text-tertiary)">
            <span v-if="docMeta.authority">{{ docMeta.authority }}</span>
            <span v-if="docMeta.authority && docMeta.date"> · </span>
            <span v-if="docMeta.date">{{ docMeta.date }}</span>
          </p>
          <p v-if="chunks.length === 1 && chunks[0].metadata?.section" class="text-xs truncate" style="color: var(--text-accent)">{{ chunks[0].metadata.section }}</p>
        </div>
      </div>
      <div class="flex items-center gap-1.5 flex-shrink-0">
        <button v-if="chunks.length === 1 && chunks[0].metadata?.document_id"
