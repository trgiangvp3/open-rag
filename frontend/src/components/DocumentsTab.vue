...
import { listDocuments, deleteDocument, getDocumentChunks, getDocumentMarkdown, uploadFile, ingestText, createCollection, deleteCollection, updateCollectionSettings, testHeadingScript, type DocumentInfo, type DocumentChunk, type TestScriptResponse } from '../api'
...
async function loadContent() {
  ...
  try {
    ...
    else chunks.value = (await getDocumentChunks(selectedDoc.value.id, collection.value)).data.chunks
        ...
        <div v-if="selectedDoc" class="w-[45%] flex-shrink-0 flex flex-col min-w-0 th-elevated border th-border rounded-r-xl overflow-hidden">
          ...
          <div v-else class="flex-1 overflow-y-auto">
            <div v-for="(chunk, i) in chunks" :key="chunk.id" class="border-b th-border2">
              ...
              <div class="px-4 py-3"><div class="prose prose-invert prose-sm max-w-none" v-html="renderMd(chunk.text)" /></div>
            </div>
          </div>
        </div>
      </Transition>
